...
The desktop application is an optional Qt Widgets front end.  The core
installation remains headless and does not import PySide6:
...
For GUI development and the Qt test fixtures:
...
The equivalent convenience command is `pathview-plus gui`.  If the optional
dependency is not installed, the command prints an installation hint rather
than changing the behaviour of the existing CLI commands.  See
[`docs/gui.md`](docs/gui.md) for the complete KEGG, SBGN, offline, cache, and
...
pathview-plus info
pathview-plus gui                         # launch the optional desktop app
```
