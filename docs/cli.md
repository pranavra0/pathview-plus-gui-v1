...
Sub-commands: `render`, `gui`, `species`, `search`, `download`, `legend`,
`parity`, `info`.
...
Launches the PySide6 desktop interface. Install the optional dependency with
`pip install "pathview-plus[gui]"`. Toggle **Offline mode** in the GUI before
rendering to prevent network access.
