...
from .input_io import read_molecular_table
...
def _read_table(path: str | Path, label: str) -> pl.DataFrame:
    """Read a molecular table, preserving the CLI's historical messages."""
    p = Path(path)
    try:
        result = read_molecular_table(p)
    except FileNotFoundError as exc:
        raise SystemExit(f"error: {label} file not found: {p}") from exc
    except ValueError as exc:
        message = str(exc)
        if message.startswith("no numeric value column"):
            raise SystemExit(f"error: {message}") from exc
        if message.endswith(
            "column(s); an identifier column plus at least one value column is required."
        ):
            raise SystemExit(f"error: {label} file {p.name} has {message}") from exc
        raise SystemExit(f"error: could not read {label} file {p}: {message}") from exc

    if result.ignored_columns:
        print(
            f"[pathview] ignoring non-numeric column(s) in {p.name}: "
            f"{', '.join(sorted(result.ignored_columns))}",
            file=sys.stderr,
        )
    return result.data
...
def _parse_limit(text: str | None) -> float | dict | None:
    ...
    for part in text.split(","):
        ...
        if key not in ("gene", "cpd"):
            raise SystemExit(f"error: --limit key must be gene or cpd, got {key!r}")
        out[key] = float(val)
    return out
    ...
    for pathway_id in args.pathway_id:
        try:
            res = pathview(
                ...
                dpi=args.dpi,
                gene_color=args.gene_palette,
                cpd_color=args.cpd_palette,
                limit=_parse_limit(args.limit),
                bins=args.bins,
                both_dirs=not args.one_sided,
                discrete=args.discrete,
                node_sum=args.node_sum,
                map_null=args.map_null,
                split_group=args.split_group,
                expand_node=args.expand_node,
                min_nnodes=args.min_nodes,
                rand_seed=args.seed,
...
def cmd_gui(args: argparse.Namespace) -> int:
    """Launch the optional Qt desktop application without importing Qt here."""
    try:
        from pathview.gui.app import main as gui_main
...
def cmd_legend(args: argparse.Namespace) -> int:
    ...
    if args.standard == "sbgn":
        ...
        kegg_legend(
            legend_type=args.legend_type, out_path=out, theme=args.theme, dpi=args.dpi
        )
    print(out)
    return 0
...
def cmd_download(args: argparse.Namespace) -> int:
    ...
    for pid in args.pathway_id:
        try:
            result = download_pathway(pid, output_dir=args.out_dir, species=args.species)
            print(f"{pid}: {result}")
        except PathviewError as exc:
...
def cmd_sbgn(args: argparse.Namespace) -> int:
    ...
    if args.list or not args.pathway_id:
        ...
        if not args.source and not args.query:
            ...
            for src, n in sorted(info["by_source"].items(), key=lambda kv: -kv[1]):
...
def cmd_info(args: argparse.Namespace) -> int:
    ...
    for meta in DATABASE_INFO.values():
        ...
        print(
            f"  {meta['name']:<18} {mark:<12} e.g. {meta['example']:<14} ({meta['source']})"
        )
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--version", action="version", version=f"pathview-plus {pathview.__version__}"
    )
    sub = parser.add_subparsers(dest="command", required=True)
    gui = sub.add_parser("gui", help="launch the optional desktop GUI")
    gui.set_defaults(func=cmd_gui)

    # -- render -------------------------------------------------------------
    r = sub.add_parser("render", help="render one or more pathways")
    r.add_argument("pathway_id", nargs="+", help="KEGG id(s), e.g. 00020 or hsa00020")
    r.add_argument(
        "--species",
        "-s",
        default="hsa",
    ...
    r.add_argument("--output-format", "-f", default="png", choices=["png", "pdf", "svg"])
    r.add_argument(
        "--theme", default="publication", choices=["publication", "slate", "dark"]
    )
    r.add_argument("--gene-palette", default=None, help="palette name for transcript data")
    r.add_argument("--cpd-palette", default=None, help="palette name for metabolite data")
    r.add_argument("--limit", default=None, help="scale limit: '1.5' or 'gene=2,cpd=1'")
    r.add_argument("--bins", type=int, default=10)
    r.add_argument(
        "--one-sided", action="store_true", help="map 0..limit instead of -limit..+limit"
    )
    r.add_argument("--discrete", action="store_true")
    r.add_argument(
        "--node-sum",
        default="sum",
        choices=["sum", "mean", "median", "max", "min", "max_abs", "random", "first"],
    )
    r.add_argument(
        "--split-group", action="store_true", help="replace complexes with their subunits"
    )
    r.add_argument(
        "--expand-node",
        action="store_true",
    ...
    r.add_argument(
        "--map-null", action="store_true", help="draw the diagram even with no data"
    )
    r.add_argument("--min-nodes", type=int, default=3)
    r.add_argument("--seed", type=int, default=None, help="seed for --node-sum random")
    ...
    r.add_argument(
        "--link-edges", action="store_true", help="also draw edges to pathway-link nodes"
    )
    r.add_argument("--no-key", action="store_true", help="omit colour keys")
    r.add_argument("--offline", action="store_true", help="never attempt a network request")
    ...
    lg.add_argument("--legend-type", default="both", choices=["both", "edge", "node"])
    ...
    sb.add_argument(
        "pathway_id", nargs="*", help="collection id(s) or path(s) to local .sbgn files"
    )
    ...
    sb.add_argument(
        "--limit", default=None, help="colour scale limit: '1.5' or 'gene=2,cpd=1'"
    )
    ...
    sb.add_argument("--output-format", "-f", default="png", choices=["png", "pdf", "svg"])
    sb.add_argument(
        "--theme", default="publication", choices=["publication", "slate", "dark"]
    )
