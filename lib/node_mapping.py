...
class NodeMapResult:
    ...
    def summary(self) -> str:
        ...
        return (
            f"{self.n_nodes_with_data}/{self.n_nodes} nodes carry data; "
            f"{self.n_ids_mapped}/{self.n_ids_input} input IDs used"
        )

    def to_dict(self) -> dict:
        """Return structured, JSON-friendly mapping diagnostics."""
        return {
...
def _explode_names(node_data: pl.DataFrame) -> pl.DataFrame:
    ...
    if "kegg_names" not in node_data.columns:
        return (
            node_data.with_columns(pl.col("name").str.split(" ").alias("kegg_names"))
            .explode("kegg_names")
            .with_columns(pl.col("kegg_names").str.replace(r"^[A-Za-z][A-Za-z0-9_]*:", ""))
        )
...
def node_map(
    ...
    try:
        summed = mol_sum(
            mol_data, id_map, sum_method=node_sum, rand_seed=rand_seed, detailed=True
        )
    except MappingError:
        res = NodeMapResult(None, n_nodes=targets.height, n_ids_input=mol_data.height)
    ...
    all_mapped = (
        exploded.filter(pl.col("kegg_names").is_in(list(input_ids)))
        .group_by("entry_id")
        .agg(pl.col("kegg_names").unique().sort().str.join(",").alias("all_mapped"))
    )

    plot_data = targets.join(values, on="entry_id", how="left").join(
        all_mapped, on="entry_id", how="left"
    )
