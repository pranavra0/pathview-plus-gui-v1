...
def pathview(
    ...
    **kwargs,
        ...
        for pid in ids:
            try:
                res = pathview(
                    ...
                    cpd_data=cpd_data,
                    species=species,
                    gene_idtype=gene_idtype,
                    cpd_idtype=cpd_idtype,
                    kegg_dir=kegg_dir,
                    out_dir=out_dir,
    ...
    needed = [
        t
        for t, p in (("xml", xml_path), ("png", png_path))
        if not p.exists() and (t == "xml" or want_png)
    ]
    if needed:
        status = download_kegg(
            numeric_id, species=code, kegg_dir=kegg_dir, file_type=needed
        )
    ...
    mappable = node_data.filter(pl.col("x").is_not_null() & pl.col("y").is_not_null())
    if mappable.height < min_nnodes:
        warnings.warn(
            f"{pathway_name}: only {mappable.height} positioned nodes "
            f"(min_nnodes={min_nnodes}); skipping.",
            stacklevel=2,
        )
        return PathwayResult(pathway_id=numeric_id, pathway_name=pathway_name, species=code)
    node_data = mappable

    # ---- map data onto nodes ----------------------------------------------
    diagnostics: dict = {
        "species": info.display_name,
        "nodes": node_data.height,
    ...
    gres = node_map(
        gene_data, node_data, gene_types, node_sum, rand_seed=rand_seed, detailed=True
    )
    cres = node_map(
        cpd_data, node_data, "compound", node_sum, rand_seed=rand_seed, detailed=True
    )
    ...
    if (
        ...
        and cpd_data is not None
        and gres.n_nodes_with_data == 0
        and cres.n_nodes_with_data == 0
    ):
        raise MappingError(
    ...
    plot_title = (
        ...
        else (f"{pathway.title or pathway_name} — {info.display_name}")
    )
...
def _remap_edges(
    edge_data: pl.DataFrame | None, node_data: pl.DataFrame
) -> pl.DataFrame | None:
    """
...
def _convert(
    data: pl.DataFrame, fn, idtype: str, code: str, node_sum: SumMethod, log
) -> pl.DataFrame:
    res = fn(
        data[data.columns[0]].cast(pl.String).to_list(),
        category=idtype,
        org=code,
        detailed=True,
    )
...
def _convert_cpd(data: pl.DataFrame, idtype: str, node_sum: SumMethod, log) -> pl.DataFrame:
    res = cpd_id_map(
        data[data.columns[0]].cast(pl.String).to_list(),
        in_type=idtype,
        out_type="KEGG",
...
def _colors(plot_data: pl.DataFrame | None, scale: ColorScale):
    ...
    if not vcols:
        return None
    return node_color(
        plot_data.rename({"entry_id": "id"}), scale, id_col="id", value_cols=vcols
    )
...
def _render(*, mode: str, output_format: str, **kw) -> Path:
    ...
    if mode == "native":
        ...
        return keggview_native(
            plot_data_gene=kw["plot_gene"],
            cols_gene=kw["cols_gene"],
            plot_data_cpd=kw["plot_cpd"],
            cols_cpd=kw["cols_cpd"],
            node_data=kw["node_data"],
            pathway_name=kw["pathway_name"],
            kegg_dir=kw["kegg_dir"],
            out_dir=kw["out_dir"],
            out_suffix=kw["out_suffix"],
            gene_scale=kw["gene_scale"],
            cpd_scale=kw["cpd_scale"],
            title=kw["title"],
            new_signature=kw["new_signature"],
            plot_col_key=kw["plot_col_key"],
            dpi=kw["dpi"],
            output_format=output_format,
        )
    if mode == "svg":
        ...
        return keggview_svg(
            node_data=kw["node_data"],
            edge_data=kw["edge_data"],
            color_map=kw["color_map"],
            label_map=kw["label_map"],
            pathway_name=kw["pathway_name"],
            title=kw["title"],
            out_dir=kw["out_dir"],
            out_suffix=kw["out_suffix"],
