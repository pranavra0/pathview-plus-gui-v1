...
def sbgn_node_map(
    ...
    for src_type, src_ids in candidates:
        for target_type in ("pathwaycommons", "chebi", "kegg", "symbol", "entrez", "ko"):
            try:
    ...
    bridge = (
        best.rename({best.columns[0]: id_col, conv_col: "kegg_names"})
        .drop_nulls()
    ...
    summed = mol_sum(
        mol_data, bridge, sum_method=node_sum, rand_seed=rand_seed, detailed=True
    )
...
def sbgnview(
    ...
    *,
    ...
    if not isinstance(pathway_id, (str, Path)) and isinstance(
        pathway_id, (list, tuple, set)
    ):
        results, failures = {}, {}
    ...
    gres = sbgn_node_map(
        gene_data, node_data, gene_idtype, _GENE_TYPES, node_sum, rand_seed, detailed=True
    )
    cres = sbgn_node_map(
        cpd_data, node_data, cpd_idtype, _CPD_TYPES, node_sum, rand_seed, detailed=True
    )
    ...
    diagnostics = {
        "glyphs": len(pathway.glyphs),
        "arcs": len(pathway.arcs),
